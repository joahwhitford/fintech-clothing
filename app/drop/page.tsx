...
const singles = [
  { slug: "dont-fuck-it-up",  name: "Don't Fuck It Up",                 teePrice: 89, hatPrice: 29, teeImage: "/drop-dfiu.png",  hatImage: "/drop-dfiu-hat.png",  description: "One rule. No exceptions. 420gsm washed black heavyweight tee.", index: 0 },
  { slug: "never-give-up",    name: "Don't Give Up",                     teePrice: 89, hatPrice: 29, teeImage: "/drop-ngu.png",   hatImage: "/drop-ngu-hat.png",   description: "The only thing between you and the outcome. Garment-dyed, pre-shrunk, built to last.", index: 1 },
  { slug: "long-term-vision", name: "Long Term Vision",                  teePrice: 89, hatPrice: 29, teeImage: "/drop-ltv.png",   hatImage: "/drop-ltv-hat.png",   description: "Everyone's optimizing for the quarter. You're building for the decade. Washed black boxy tee.", index: 2 },
  { slug: "great-things",     name: "Great Things Don't Come Overnight", teePrice: 89, hatPrice: 29, teeImage: "/drop-gtco.png",  hatImage: "/drop-gtco-hat.png",  description: "Patience is the edge nobody talks about. Italian fleece heavyweight tee.", index: 3 },
];
    ...
    <div className="bg-[#0a0f1a] pb-2">
      <div className="mx-auto max-w-[1500px] px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <span className="font-logo text-xs uppercase tracking-logo text-[#f6f7f5]/35">4 tee colorways + hat — from $29</span>
        </motion.div>
...
function SingleCard({ item, onAdd }: {
  ...
  return (
    <motion.div
      ...
      className="grid gap-px overflow-hidden border border-[#2d5ca2]/15 bg-[#2d5ca2]/15 lg:grid-cols-[1fr_1fr]"
    >
      <div className={`relative overflow-hidden bg-[#edf0f1] ${item.index % 2 !== 0 ? "lg:order-last" : ""}`}>
        <AnimatePresence mode="wait">
          <motion.img key={view} src={image} alt={item.name}
            initial={{ opacity: 0, scale: 1.03 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}
            transition={{ duration: 0.35 }} className="h-full w-full object-cover" style={{ minHeight: "420px" }} />
        </AnimatePresence>
      ...
      <div className="flex flex-col justify-between bg-[#f7f8f6] p-8 md:p-12">
        <div>
          ...
          <p className="mt-6 max-w-sm font-logo text-sm leading-6 text-[#2d5ca2]/60">{item.description}</p>
          <div className="mt-8 inline-flex overflow-hidden border border-[#2d5ca2]/20">
            {(["tee", "hat"] as const).map((type) => (
              <button key={type} onClick={() => setView(type)}
                className={`px-5 py-2 font-logo text-xs uppercase tracking-logo transition ${view === type ? "bg-[#2d5ca2] text-[#f6f7f5]" : "bg-transparent text-[#2d5ca2]/55 hover:text-[#2d5ca2]"}`}>
        ...
        <div className="mt-10 space-y-4">
          ...
          <button
            onClick={() => onAdd({ slug: `${item.slug}-${view}`, name: `${item.name} — ${isTee ? "Tee" : "Hat"}`, price, image, isHat: !isTee })}
